...
pub fn init(allocator: std.mem.Allocator, window: *c.SDL_Window, renderer: *c.SDL_Renderer, state_machine: *Statemachine) !*Self {
    ...
    ptr_piece.* = try Piece.randomPiece(self.renderer, &self.score, &self.line); // Need to do this, so Board is allocated on the Heap
...
fn renderFn(child: *StateInterfce) !void {
    ...
    var txt_width = self.font_info.calculateTextWidth("Level");
    ...
    var line_txt = try std.fmt.allocPrintZ(self.allocator, "{d}", .{self.line});
